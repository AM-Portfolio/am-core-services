...
/**
 * [WHY THIS WAS ADDED]:
 * Zero-Configuration bridge for Lettuce Redis. If any microservice defines
 * their own
 * custom {@link LettuceConnectionFactory} bean, Spring Boot's default client
 * configuration
 * is bypassed. This PostProcessor intercepts the factory bean *before*
 * initialization
 * (before afterPropertiesSet) and programmatically overrides its
 * ClientResources
 * configuration using a Dynamic Proxy to inject tracing.
 *
 * [WHY postProcessBeforeInitialization]:
 * LettuceConnectionFactory implements InitializingBean. During
 * afterPropertiesSet() it
 * builds an internal connection pool using whatever ClientResources are
 * configured at
 * that moment. If we inject tracing after afterPropertiesSet
 * (postProcessAfterInitialization),
 * the connection pool is already built with an untraced ClientResources — Redis
 * spans
 * won't appear in Tempo. Using postProcessBeforeInitialization guarantees our
 * traced
...
public class LettuceConnectionFactoryTracingPostProcessor implements BeanPostProcessor {
    ...
    public LettuceConnectionFactoryTracingPostProcessor(
            org.springframework.beans.factory.ObjectProvider<ClientResources> clientResourcesProvider) {
     ...
     * LettuceClientConfiguration before Lettuce builds its internal connection
     * pool.
     */
    @Override
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        if (bean instanceof LettuceConnectionFactory factory) {
            try {
                log.info(
                        "[OBSERVABILITY] Intercepted LettuceConnectionFactory bean '{}' BEFORE initialization to inject tracing ClientResources",
                        beanName);

                ClientResources clientResources = clientResourcesProvider.getIfAvailable();
                if (clientResources == null) {
                    log.warn(
                            "[OBSERVABILITY] ClientResources bean not available yet for '{}', skipping tracing injection",
                            beanName);
                    return bean;
                }

                // Extract the LettuceClientConfiguration field (set before afterPropertiesSet
                // builds the pool)
                ...
                if (originalConfig == null) {
                    log.warn(
                            "[OBSERVABILITY] LettuceClientConfiguration was null in factory bean '{}', skipping tracing injection",
                            beanName);
                    return bean;
                }
                ...
                if (currentResources.isPresent() && currentResources.get() == clientResources) {
                    log.debug(
                            "[OBSERVABILITY] LettuceConnectionFactory '{}' already has traced ClientResources, skipping",
                            beanName);
                    return bean;
                }
                ...
                log.info(
                        "[OBSERVABILITY] Successfully injected tracing ClientResources into LettuceConnectionFactory '{}' BEFORE pool creation",
                        beanName);

            } catch (NoSuchFieldException e) {
                log.error(
                        "[OBSERVABILITY] Could not find 'clientConfiguration' field in LettuceConnectionFactory '{}' — Redis spans may be missing. Spring Data Redis internal structure may have changed.",
                        beanName, e);
            } catch (IllegalAccessException e) {
                log.error(
                        "[OBSERVABILITY] Could not access 'clientConfiguration' field in LettuceConnectionFactory '{}'",
                        beanName, e);
            } catch (Exception e) {
                log.error("[OBSERVABILITY] Unexpected error injecting tracing into LettuceConnectionFactory '{}'",
                        beanName, e);
            }
        }
        return bean;
