 ...
 * - It only emits USER_WATCHING events (via PortfolioSubscriptionManager →
 * Kafka).
 * - The DemandDrivenOrchestrator (am-analysis) is the SOLE publisher of
 * TRIGGER_CALCULATION.
 *
 * Flow:
 * /subscribe → register in Redis + emit USER_WATCHING → Orchestrator decides to
 * trigger
 * /heartbeat → renew Redis TTL (keeps user alive)
 * /unsubscribe → remove from Redis + emit UNSUBSCRIBE event
 * /calculate → REMOVED: No longer needed. Subscribe now drives the trigger
 * pipeline.
 */
@Controller
...
public class PortfolioController {
     ...
     *             The Orchestrator will immediately trigger a calculation on first
     *             subscription.
     *
     *             If this breaks any existing client, redirect them to
     *             /portfolio/subscribe.
     */
    ...
    public void triggerCalculation(@Payload Map<String, String> payload, SimpMessageHeaderAccessor headerAccessor) {
        ...
        // Redirect to the subscribe pipeline — Orchestrator will trigger calc
        // automatically.
