...
public class AnalysisAggregator {
    ...
    public Map<String, String> resolveIsinDisplayTickers(List<AnalysisEntity> entities) {
        ...
        List<String> isins = entities.stream()
                ...
                .filter(Objects::nonNull)
                .map(id -> {
                    if (LivePriceOverlayHelper.looksLikeIsin(id.getSymbol())) {
                        return id.getSymbol().trim().toUpperCase();
                    }
                    if (LivePriceOverlayHelper.looksLikeIsin(id.getIsin())) {
        ...
        try {
            ...
            log.warn("[Aggregator] ISIN display resolution failed: {}", e.getMessage());
            return Map.of();
        }
    }
