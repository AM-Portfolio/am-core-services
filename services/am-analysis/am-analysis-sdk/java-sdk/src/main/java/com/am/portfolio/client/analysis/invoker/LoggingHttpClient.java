package com.am.portfolio.client.analysis.invoker;
...
public class LoggingHttpClient extends HttpClient {
    ...
    public <T> CompletableFuture<HttpResponse<T>> sendAsync(HttpRequest request,
        ...
        // Wrap Future? Or convert?
        // Basic implementation: Log request now, log response when future completes.
        long start = System.currentTimeMillis();
        logRequest(request);
    ...
    private String getRequestBody(HttpRequest.BodyPublisher publisher) {
        try {
            ...
            String[] possibleFields = { "content", "s", "bytes", "is" };
            ...
            // For ByteArrayPublisher
            return "[body]"; // Simplification as reading publisher is tricky without fully subscribing
        } catch (Exception e) {
    ...
    private <T> HttpResponse<T> logResponse(HttpResponse<T> response, long duration) {
        ...
        if (body instanceof InputStream) {
            // Only buffer if debug enabled to save memory
            if (logger.isDebugEnabled()) {
                byte[] bytes = null;
